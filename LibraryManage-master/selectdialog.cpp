...
SelectDialog::SelectDialog(QWidget *parent, int _bookID, int _userID) ://选择对话框
    QDialog(parent),
...
void SelectDialog::initBookTable() {//初始化图书表
    bookModel->clear();
...
void SelectDialog::initUserTable() {//初始化用户表
    userModel->clear();
...
void SelectDialog::displayBookData() {//显示图书数据
    initBookTable();
...
void SelectDialog::displayUserData() {//显示用户数据
    initUserTable();
...
void SelectDialog::appendSingleBook(Node<BookInfo>* p) {//借阅图书
    if (!p) return;
...
void SelectDialog::appendSingleUser(Node<UserInfo>* p) {//添加图书借阅者
    if (!p) return;
    QList<QStandardItem*> list;
    list << new QStandardItem(p->elem.name.data())
         << new QStandardItem(std::to_string(p->elem.identifier).data())
